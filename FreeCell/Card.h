...
*
*		operator !=(const Card & rhs)
*			Returns true if the compared cards do not have the same suit and rank.
*
*		SetRank(Rank rank)
*			Sets m_rank to rank.
*		SetSuit(Suit suit)
...
private:
	...
	int m_type;
public:
	Card(Rank rank = ACE, Suit suit = SPADES, int type = 0);
	Card(const Card & copy) { *this = copy; }
	...
	int GetSide() { return m_type; }			// 0 - faceup; 1 - nocard
